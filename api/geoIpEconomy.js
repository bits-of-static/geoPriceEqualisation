...
/*******************************************************************************************/
ihde.GeoIpEconomy = function(isoCode) {
	this.isoCode = isoCode || '-';
	if (!(this.isoCode in ihde.GeoIpEconomy.data)) {
		this.isoCode = '-';
	} //bottom
	
	var data = ihde.GeoIpEconomy.data[this.isoCode]
	
	this.name = data.name;
	this.factor = data.factor;
	this.percentual = (this.factor- ihde.GeoIpEconomy.min)/ (ihde.GeoIpEconomy.max- ihde.GeoIpEconomy.min);
}; //routine

/*******************************************************************************************/
ihde.GeoIpEconomy._static_initializer = function() {
	var min = Infinity;
	var max = -Infinity;
	for (var i in ihde.GeoIpEconomy.data) {
		var data = ihde.GeoIpEconomy.data[i];
		min = Math.min(min, data.factor);
		max = Math.max(max, data.factor);
	} //for
	
	ihde.GeoIpEconomy.min = min;
	ihde.GeoIpEconomy.max = max;
}; //routine
...
/*******************************************************************************************/
ihde.GeoIpEconomy.prototype.readapt = function(originIsoCode, price) {
	var normalizedPrice = price/ ihde.GeoIpEconomy.data[originIsoCode].factor;
	return this.adapt(normalizedPrice);
} //routine

/*******************************************************************************************/
ihde.GeoIpEconomy.data = {
	"-": {
